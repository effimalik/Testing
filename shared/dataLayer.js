...
(function () {
  ...
  /* ─────────────────────────────────────────
     CONFIG — comes from shared apiConfig.js (see below), not hardcoded here
  ───────────────────────────────────────── */
  // API_BASE now comes from the shared Reg/shared/apiConfig.js file (must be
  // loaded via <script src="Reg/shared/apiConfig.js"></script> BEFORE this
  // file, in every page that includes dataLayer.js). No URL is hardcoded
  // here anymore — update it in ONE place (apiConfig.js) and every page
  // that reads data through this file picks it up automatically.
  ...
  const OBJECT_SHAPE_DATASETS = new Set(['ap2_loginData', 'ap2_ticLog']);
  ...
  async function _fetchFromServer(dsKey) {
    ...
    const promise = (async () => {
      ...
      const url = `${API_URL}?${paramKey}=${encodeURIComponent(ds.apiKey)}`
  ...
  window.AdminPro = {
    ...
    // Returns { ticLog: [...], Ticket: [...] } — object-shape dataset, not a flat array
    getTicLog     (force) { return _get('ap2_ticLog',   force); },
